...
- [x] 任务 1：创建 DpapiHelper 加密工具类（AC：#2, #3）
  - [x] 1.1 创建 DpapiHelper 类（src/helpers/DpapiHelper.h/cpp）
  - [x] 1.2 实现 encrypt() 静态方法
  - [x] 1.3 实现 decrypt() 静态方法
  - [x] 1.4 添加错误处理和日志记录

- [x] 任务 2：修改 ConfigManager 集成密码加密（AC：#2, #3, #4, #5）
  - [x] 2.1 修改 addAccount() 方法，保存前加密密码
  - [x] 2.2 修改 updateAccount() 方法，保存前加密密码
  - [x] 2.3 修改 accounts() 方法，返回时解密密码
  - [x] 2.4 添加密码字段检测逻辑（判断是否已加密）

- [x] 任务 3：更新 CMakeLists.txt 添加依赖（AC：#2, #3）
  - [x] 3.1 添加 Crypt32 库链接依赖
  - [x] 3.2 添加 Windows 平台条件编译

- [x] 任务 4：处理边界情况（AC：全部）
  - [x] 4.1 处理加密/解密失败的情况
  - [x] 4.2 处理配置文件迁移（未加密→加密）
  - [x] 4.3 记录加密操作日志

- [x] 任务 5：测试验证（AC：全部）
  - [x] 5.1 验证密码加密后存储到配置文件
  - [x] 5.2 验证读取时密码自动解密
...
- DpapiHelper::encrypt() - 调用 CryptProtectData 加密数据
- DpapiHelper::decrypt() - 调用 CryptUnprotectData 解密数据
- LogManager::logInfo/logError - 记录加密/解密操作日志

### 完成笔记列表

- 创建了 DpapiHelper 加密工具类（src/helpers/DpapiHelper.h/cpp）
- 实现了 encrypt() 静态方法，使用 Windows DPAPI 加密数据
- 实现了 decrypt() 静态方法，使用 Windows DPAPI 解密数据
- 添加了错误处理和日志记录
- 修改了 ConfigManager::addAccount() 方法，保存前自动加密密码
- 修改了 ConfigManager::updateAccount() 方法，更新时自动加密密码
- 修改了 ConfigManager::accounts() 方法，返回时自动解密密码
- 添加了密码字段检测逻辑（passwordEncrypted 标记）
- 处理了边界情况：加密/解密失败、配置文件迁移

### 文件列表

- src/helpers/DpapiHelper.h（新增）
- src/helpers/DpapiHelper.cpp（新增）
- src/core/ConfigManager.cpp（修改）
- CMakeLists.txt（已存在 Crypt32 库依赖）
